...
int sum(int u, int l, int r)  //求区间和
{
    ...
    int mid = tr[u].l + tr[u].r >> 1;  //如果l和r不包括区间，即节点u的区间大于[l, r]，              则先计算出线段树区间中点，注意不是l和r的中点
    int s = 0;  //返回值
    if (l <= mid) s =  sum(u << 1, l, r); //l和左半边有交集，加上左半边的和
    if (r > mid) s += sum(u << 1 | 1, l, r); //r和右半边有交集，加上右半边的和
